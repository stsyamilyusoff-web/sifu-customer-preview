/* Sifututor design-review widget.
   ...
   Finds every .pair (a before/after design block), injects a comment box + OK/Needs-work
   under it, and adds a floating "Copy all feedback / Email" bar that gathers comments
   from every page (shared localStorage). No per-screen editing needed. */
(function () {
  ...
  var state = load();
  var pageSlug = (location.pathname.split("/").pop() || "index").replace(/\.html$/,"") || "index";

  var css = ""
   + ".rvw{max-width:600px;margin:10px auto 30px;background:#f3f6ff;border:1px solid #d6deff;border-left:3px solid #243cb3;border-radius:12px;padding:13px 15px;font-family:-apple-system,system-ui,sans-serif}"
  ...
  pairs.forEach(function(pair, i){
    var label = nearestSech(pair) || ("Screen block " + (i + 1));
    var key = pageSlug + "#" + i;
    var cur = state[key] || {};
    ...
    function persist(patch){
      ...
      c.page = pageSlug; c.label = label; state[key] = c; save();
